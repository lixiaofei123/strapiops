            ...
            <tr v-for="item in items" v-bind:key="item.id.data">
                <td v-for="field in fields" v-bind:key="field.key">
                    ...
                    <div v-if="item[field.key].type === 'array'">
                        <div v-for="(item, index) in item[field.key].data" v-bind:key="index" style="float: left;margin-right: 10px;">
                ...
                <td>
                    <div>
                        <CRow class="align-items-center remove-padding">
                            <CCol col="6">
                                <CButton block color="secondary" size="sm" @click="$emit('editContent', item.id.data)">编辑</CButton>
                            </CCol>
                            <CCol col="6">
                                <CButton block color="danger" size="sm" @click="$emit('deleteContent', item.id.data)">删除</CButton>
                            </CCol>
                        </CRow>
                    </div>
                </td>
